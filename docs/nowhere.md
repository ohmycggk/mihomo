...
`mihomo` 内置 Nowhere 出站与入站（`type: nowhere`）。协议核心内联于 `transport/nowhere/core`，对齐 [`nowhere-go`](https://github.com/ohmycggk/nowhere-go) **v2.0.0** / Rust Nowhere **v2.0.0**（`UPSTREAM.lock` commit `0453efd`）。服务端也可对接 `sing-box` inbound 或 Rust Portal，对端必须同样运行 Nowhere 2。

Nowhere 2 的唯一 ALPN 是 `nw2`。认证盐、Mux 帧与 QUIC UDP 头均已更换，**不能**与 1.8 `now/1` 对端完成握手。

客户端可独立选择上行 / 下行外层载体：`tcp`（TLS/TCP）、`udp`（QUIC/UDP）或 `mix`（客户端策略，写入 FlowHeader 之前按 flow 解析）。对称矩阵走直通快路径，非对称矩阵由 Portal 按 `(session_id, flow_id)` 配对。`mix` 不会出现在线协议里。
...
省略载体字段时默认 `up: udp` + `down: udp`（QUIC）。省略 `alpn` 时使用协议默认 `nw2`。`password` 为必填字段，省略时启动报错。入站始终同时监听 TCP 与 UDP（同端口 mix）。
...
| `password` | — | 共享密钥（与 trojan/anytls/tuic 一致），1–255 字节；省略时启动报错 `missing password` |
...
填写 `next` 后，本节点作为中继把**全部**入站 flow 转发给下一个 Nowhere Portal，绕过 mihomo 路由 / `rule` / `proxy`，无回退。`next` 可使用 `mux` 与 `mix`。链上每个 Portal 必须运行 Nowhere **2.0**（ALPN `nw2`）。
...
| 字段 | 默认 | 说明 |
| --- | --- | --- |
| `next.server` / `port` / `password` | — | 下一跳地址、端口（1–65535）、共享密钥（必填，1–255 字节） |
...
- **2.0 必须锁步**：ALPN `nw2`、AuthFrame 盐、Mux 帧与 QUIC UDP 头均与 1.8 不兼容。混跑 1.8 `now/1` 无法完成握手。
- **Mux**：`mux=0` 专用客户端可对接 2.0 Portal；Portal 自动识别两种 TLS 成帧。同一 session 最多 8 条 Mux TLS 载体。
- **mix** 只存在于客户端；线协议仍是四矩阵。对端 mix 入站即可。
